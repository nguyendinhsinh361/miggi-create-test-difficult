...
def get_question_by_ids_kind_and_add_score(ids_diff, ids_not_diff, kind, score, total_ques):
    all_question_in_kind = total_ques[0] + total_ques[1]
    result = []
    cursor = CONN.cursor()

    limit_ques_diff = total_ques[0]
    query_1 = "SELECT * FROM questions WHERE id IN ({})".format(
        ','.join(map(str, ids_diff)))
    query_completed_1 = f'{query_1} AND kind = "{kind}" ORDER BY RAND() LIMIT {limit_ques_diff}'

    query_2 = "SELECT * FROM questions WHERE id NOT IN ({})".format(
        ','.join(map(str, ids_diff)))
    query_completed_2 = f'{query_2} AND kind = "{kind}" ORDER BY RAND() LIMIT {total_ques[1]}'
    ...
    if (all_question_in_kind != len(raw_data)):
        query_completed_final = f'{query_2} AND kind = "{kind}" ORDER BY RAND() LIMIT {all_question_in_kind}'
    cursor.execute(query_completed_final)
    raw_data = cursor.fetchall()
    for tmp in raw_data:
        obj: QuestionModel = {
